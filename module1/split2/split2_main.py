...
def main() -> None:
    ...
    # FIXED:
    _base = SERVER_ADDRESS.rsplit(":", 1)[0]

    # Local subprocess clients must connect to 127.0.0.1, not 0.0.0.0
    # 0.0.0.0 is valid for the server to LISTEN on all interfaces
    # but NOT valid for clients to CONNECT to
    if _base == "0.0.0.0":
        client_address = f"127.0.0.1:{args.port}"   # local clients use loopback
    else:
        client_address = f"{_base}:{args.port}"      # local mode unchanged

    server_address = f"{_base}:{args.port}"          # server still binds to 0.0.0.0
