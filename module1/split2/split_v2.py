...
# governance_bridge resolves split3/ import paths and builds the GovernanceEngine.
# If split3/ is not found or --no_governance is passed, it returns None safely
# and Module 1 runs exactly as before with no governance.
try:
    from common.governance_bridge import build_governance_engine
    _GOVERNANCE_AVAILABLE = True
except ImportError:
    _GOVERNANCE_AVAILABLE = False
    print("[Main] governance_bridge not found — Module 2 disabled.")
...
ATTACK EXAMPLES
  ...
  NOTE: Running --attack twice is NOT supported by argparse (only last value kept).
    ...
    p.add_argument("--no_governance", action="store_true",
                   help="Disable Module 2 blockchain governance (Module 1 only)")
...
def main() -> None:
    ...
    # server_address: what the Flower server binds to
    #   "0.0.0.0:8081" = listen on all interfaces (network mode)
    #   "127.0.0.1:8081" = loopback only (local mode)
    server_address = f"{_base}:{args.port}"

    # client_address: what LOCAL subprocess banks connect to
    #   CRITICAL: 0.0.0.0 is NOT a valid connection destination on Windows
    #   Local subprocesses must always connect via 127.0.0.1 (loopback)
    #   Remote clients (System 2) use the real WiFi IP — set in run_client.py
    if _base == "0.0.0.0":
        client_address = f"127.0.0.1:{args.port}"   # local banks use loopback
    else:
        client_address = server_address              # local mode: same as server
    ...
    # ── Step 5: Build Module 2 GovernanceEngine (if enabled) ─────────────────
    governance_engine = None
    if _GOVERNANCE_AVAILABLE and not args.no_governance:
        governance_engine = build_governance_engine(
            log_dir               = args.log_dir,
            governance_output     = "governance_output",
            anomaly_threshold     = 0.5,
            consecutive_flag_limit = 3,
            use_simulation        = True,   # False = real Hyperledger Fabric
            enabled               = True,
        )
    elif args.no_governance:
        print("[Main] --no_governance flag set — running Module 1 only.")
    ...
    strategy = get_trust_strategy(
        ...
        governance_engine = governance_engine,  # None = Module 1 only
    )
    ...
    for cid in range(args.num_clients):
        ...
        cmd = [
            ...
            "--_model",      args.model,
            "--_smote",      smote_flag,
            "--_server",     client_address,   # local banks connect via 127.0.0.1
            "--_label_flip", flip_flag,
        ]
        proc = subprocess.Popen(
            cmd,
