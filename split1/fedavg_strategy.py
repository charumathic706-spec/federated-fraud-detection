...
# PURPOSE: Server-side Federated Averaging strategy for Split 1.
#
# What the server does each round:
#   1. Selects fraction_fit% of available bank clients
#   2. Broadcasts current global model weights to selected clients
#   3. Receives updated local weights back from each client
#   4. Computes sample-weighted average of all parameter arrays (FedAvg)
#   5. Logs full per-round and per-client metrics to training_log.json
#   6. Tracks best global F1 score and which round it occurred
#
# Metrics logged every round (global + per-client):
#   F1, AUC-ROC, Recall, Precision, Accuracy
#
# NOTE: This is the baseline. Split 2 replaces aggregate_fit() with
#       trust-weighted aggregation that detects malicious clients.
#       weighted_average() is also re-used directly by Split 2.
# =============================================================================
...
# =============================================================================
# WEIGHTED METRIC AGGREGATION  (also imported by Split 2)
# =============================================================================

def weighted_average(metrics: List[Tuple[int, Metrics]]) -> Metrics:
    """
    Compute a sample-count-weighted average over a list of client metric dicts.

    FIX v9: AUC-ROC is excluded from the standard weighted average and instead
    computed only over clients that have F1 > 0 (active clients). A dead client
    (F1=0, AUC=0, large test set) was dragging the weighted AUC below 0.5,
    making the global model appear worse than random even though 4/5 banks
    had individual AUCs of 0.71–0.99.

    Args:
        metrics: List of (num_examples, metric_dict) tuples — one per client.

    Returns:
        Single dict with weighted-average value per metric key.
    """
    ...
    for key in keys:
        if key == "auc_roc":
            # Weight AUC only over active (non-dead) clients
            active = [(n, m) for n, m in metrics if float(m.get("f1", 0)) > 0]
            if active:
                active_total = sum(n for n, _ in active)
                result[key] = (sum(n * float(m.get(key, 0.0)) for n, m in active)
                               / active_total)
            else:
...
class InstrumentedFedAvg(FedAvg):
    """
    Standard FedAvg with full metric logging.

    Adds on top of Flower's built-in FedAvg:
      - Per-round console output (per-client + global metrics)
      - Full JSON log written after every round (training_log.json)
      - Tracks best global F1 and the round it occurred
      - Final summary via print_summary()

    Metrics tracked per round:
      Global  : F1, AUC-ROC, Recall, Precision, Accuracy, Loss
      Per-bank: F1, AUC-ROC, Recall, Precision, Accuracy
    """
    ...
    def aggregate_fit(
        ...
        """
        RECALL-WEIGHTED FedAvg  (v7 upgrade from sample-count-weighted).

        Standard FedAvg uses w_i = n_i / N (sample count fraction).
        This biases aggregation toward clients with large datasets, which
        in v6 happened to be the high-precision/low-recall clients — causing
        the global model to drift toward precision after each round.

        New formula:
            w_i = recall_i / sum(recall_j)

        Clients with higher recall contribute more to the global update,
        pulling the aggregate weights toward recall-preserving directions.
        Fallback: if no recall metric is available (unexpected), falls back
        to standard sample-count weighting.

        The underlying Flower FedAvg aggregation is replaced by a manual
        numpy weighted-average over the raw parameter arrays.
        """
        ...
        print(
            ...
            f"{len(results)} clients responded  |  {len(failures)} failures"
        )
        print(sep)
        ...
        # ── Collect per-client recall and parameter arrays ────────────────────
        client_data = []
        ...
        if total_recall > 1e-9:
            ...
            print(f"\n  [Recall weights] " +
                  "  ".join(f"C{cd['cid']}={w:.3f}" for cd, w in
                             zip(client_data, weights)))
        else:
            # Fallback: uniform weights
            weights = [1.0 / len(client_data)] * len(client_data)
            print("  [Weights] fallback to uniform (no recall metric)")

        # ── Manual weighted average of parameter arrays ───────────────────────
        aggregated = [
        ...
        # ── Aggregate metrics (recall-weighted for consistency) ───────────────
...
def get_fedavg_strategy(
    ...
    """
    Build the FedAvg strategy instance.

    Args:
        num_clients:  Total number of bank nodes
        fraction_fit: Fraction selected per round (1.0 = all)
        log_dir:      Directory for training_log.json

    Returns:
        InstrumentedFedAvg ready for fl.simulation.start_simulation()
    """
